...
const uint32_t LEAF_NODE_NEXT_LEAF_OFFSET = LEAF_NODE_NUM_CELL_SIZE + LEAF_NODE_NUM_CELL_OFFSET;
const uint32_t LEAF_NODE_HEADER_SIZE = COMMON_NODE_HEADER_SIZE + LEAF_NODE_NUM_CELL_SIZE + LEAF_NODE_NEXT_LEAF_SIZE;
...
const uint32_t INTERNAL_NODE_RIGHT_CHILD_OFFSET = INTERNAL_NODE_NUM_KEY_SIZE + INTERNAL_NODE_NUM_KEY_OFFSET;
...
uint32_t* internal_node_child (void* node, uint32_t child_num) {
    ...
    if (child_num > num_keys) {
        ...
        if (*right_child == INVALID_PAGE_NUM) {
            printf("Tried to access right invalid right child of a node.\n");
            exit(EXIT_FAILURE);
        }
        return right_child;
    }
...
ExecutableResult execute_statement (Statement* statement, Table* table) {
    ...
    while (true) {
        ...
        if (input_buffer->buffer[0] == '.') {
            switch (do_meta_command(input_buffer, table)) {
                ...
                case (META_COMMAND_UNRECOGNIZED_COMMAND): printf("Unrecognized command %s \n.", input_buffer->buffer);
            }
        }
        ...
        switch (prepare_statement (input_buffer, &statement)) {
            ...
            case (PREPARE_NEGATIVE_ID) : printf("Negative Id Error.\n"); continue;
            ...
            case (PREPARE_UNRECOGNIZED_STATEMENT) : printf("Unrecognized keyword at the start of %s \n.", input_buffer->buffer);
            continue;
        }
